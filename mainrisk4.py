...
class ImmersiveQAApp(ctk.CTk):
    ...
    def _image_fetch_worker(self, query):
        try:
            ...
            # Attempt 2: Wikipedia Fallback (Great for famous people & concepts)
            if not image_url:
                try:
                    import wikipedia
                    ...
                    if page_titles:
                        ...
                        # Find the first image that isn't a small UI icon (like an SVG)
                        for img in page.images:
                            if not img.lower().endswith(('.svg', '.gif')):
                                image_url = img
                                break
                except Exception as wiki_e:
                    # Silently pass wikipedia errors to reach the final failure check
                    pass

            # Final Check: Did we get a URL from either source?
            if not image_url:
                self.after(0, lambda: self.append_chat(f"❌ No images found or APIs are completely rate-limited for: '{query}'", "sys_color"))
    ...
    def show_image_window(self, img, prompt):
        ...
        # Ensures the window immediately takes focus instead of appearing behind the main app
    ...
    def send_message(self):
        ...
        # 1. Image Check (Uses DuckDuckGo integration now)
        ...
        if any(kw in lower_q for kw in image_keywords):
            ...
            # Start image fetching directly, as the method now threads itself
    ...
    def fetch_ai_response(self, question):
        ...
        try:
            ...
            if "WEB_SEARCH_REQUIRED" in ai_answer:
                ...
                self.append_chat("📄 According to Local Documents...", "sys_color")
