...
# Web Search & Image Fetching Imports
...
class ImmersiveQAApp(ctk.CTk):
    def __init__(self):
        ...
        self.chat_history = []
        self.max_history_turns = 4 # Keeps the last 4 conversational turns (8 messages total)
    ...
    def reset_input_ui(self):
        ...
        self.gui_update_entry(state="normal", clear=True)
    ...
    # ==========================================
    # WEB IMAGE FETCHING & DISPLAY (UPDATED)
    # ==========================================
    def fetch_real_web_image(self, query):
        """Fetches web images with rate-limit handling and a Wikipedia fallback."""
        self.after(0, lambda: self.append_chat(f"🎨 Searching for image: '{query}'...", "success_color"))
        self.gui_set_progress(start=True)
        
        # Start this logic in a background thread
        threading.Thread(target=self._image_fetch_worker, args=(query,), daemon=True).start()

    def _image_fetch_worker(self, query):
        try:
            image_url = None
            ...
            try:
                ...
                # If we hit a 403 Rate Limit, notify the UI but don't crash
                if "403" in str(e) or "Ratelimit" in str(e):
                    self.after(0, lambda: self.append_chat("⚠️ DuckDuckGo Rate Limited. Trying Wikipedia fallback...", "sys_color"))
                else:
                    self.after(0, lambda err=e: self.append_chat(f"⚠️ Search engine warning: {err}", "sys_color"))
            ...
            # Final Check
            if not image_url:
                self.after(0, lambda: self.append_chat(f"❌ No images found or APIs are rate-limited for: '{query}'", "sys_color"))
                self.after(0, self.reset_input_ui)
                return
            ...
            self.after(0, lambda: self.show_image_window(img_data, query))
            
        except Exception as e:
            self.after(0, lambda err=e: self.append_chat(f"❌ Failed to download web image: {err}", "sys_color"))
        finally:
            self.after(0, self.reset_input_ui)

    def show_image_window(self, img, prompt):
        img_window = ctk.CTkToplevel(self)
        img_window.title(f"Image: {prompt}")
        img_window.geometry("550x550")
        ...
        lbl = ctk.CTkLabel(img_window, image=ctk_img, text="")
        lbl.pack(padx=20, pady=20, expand=True)
        
        self.append_chat("🖼️ Image loaded successfully", "sys_color")

    # ==========================================
    # CORE MEMORY & AI RESPONSE LOGIC
    # ==========================================
    def send_message(self):
        ...
        # 1. Image Check
        image_keywords = ["picture", "image", "photo", "pic", "draw", "generate image", "show me","diagram"]
        if any(kw in lower_q for kw in image_keywords):
            self.append_chat(f"\n🧑 You:\n{question}\n", "user_color")
            self.gui_update_entry(clear=True, state="disabled")
            self.gui_update(self.btn, state="disabled")
            self.gui_update(self.mic_btn, state="disabled")
            
            self.fetch_real_web_image(question)
            return
            
        # 2. YouTube check
        trigger_words_yt = ["play ", "watch ", "listen to "]
        if any(lower_q.startswith(kw) for kw in trigger_words_yt):
            self.append_chat(f"\n🧑 You:\n{question}\n", "user_color")
            self.gui_update_entry(clear=True)
            search_term = question
            for kw in trigger_words_yt:
    ...
    def fetch_ai_response(self, question):
        ...
        try:
            ...
            You are a strict, helpful AI assistant with short-term memory.
            
            RULES:
            1. Analyze the user's latest input in the context of the recent conversation history. If they reply with a short confirmation (e.g., "Yes", "Sure", "Code please"), fulfill the action you previously offered.
            2. If answering a brand new question, use ONLY the 'LOCAL DOCUMENTS CONTEXT' below.
            3. If the answer to a NEW factual question is NOT in the context, you must reply with the exact phrase: "WEB_SEARCH_REQUIRED". Do not include any other text.
            ...
            if self.chat_history:
                messages_payload.extend(self.chat_history[-(self.max_history_turns * 2):])
                
            messages_payload.append(HumanMessage(content=question))
            ...
            # Evaluate Fallback to Web Search
            if "WEB_SEARCH_REQUIRED" in ai_answer:
                if not online:
                    self.append_chat("⚠️ Information not found locally. Cannot search web offline.", "sys_color")
                    return
                
                self.append_chat("🌍 According to web ...", "sys_color")
                try: 
                    search_query = question if len(question.split()) > 2 else f"{self.chat_history[-1].content} {question}" if self.chat_history else question
                ...
                final_response = self.model.invoke([SystemMessage(content="You are a helpful assistant."), HumanMessage(content=prompt_web)])
                ai_answer = final_response.content.strip()
                self.animate_typing(ai_answer)
            else:
                self.append_chat("📄 According to Context/Memory...", "sys_color")
                self.animate_typing(ai_answer)
                
            # Log successful turns to memory
            self.chat_history.append(HumanMessage(content=question))
            self.chat_history.append(AIMessage(content=ai_answer))
